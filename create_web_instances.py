...
def credel_web_instances(key):
    ...
    mongodb = ec2_resource.create_instances(ImageId='ami-061eb2b23f9f8839c', InstanceType='t2.medium',MinCount=1,MaxCount=1,KeyName=key,SecurityGroupIds=[security_group_id])
    mysql = ec2_resource.create_instances(ImageId='ami-061eb2b23f9f8839c', InstanceType='t2.medium',MinCount=1,MaxCount=1,KeyName=key,SecurityGroupIds=[security_group_id])
    web = ec2_resource.create_instances(ImageId='ami-061eb2b23f9f8839c', InstanceType='t2.medium',MinCount=1,MaxCount=1,KeyName=key,SecurityGroupIds=[security_group_id])
